...
import 'package:audio_processing/screens/results_1_screen.dart';
...
class AllModePage extends StatefulWidget {
  @override
  _AllModePageState createState() => _AllModePageState();
}

class _AllModePageState extends State<AllModePage> {
  ...
  void _listenToResultItemChanges() {
    ...
    // Listen for new result items (Insert)
      ...
      callback: (payload) {
        ...
        // Define your own UUID here (assuming it's a String)

        if (newRecord.isNotEmpty) {
          // Check if the UUID matches your own UUID
          if (newRecord['uuid'] == globUuuid && globUuuid != '') {
            setState(() {
              ...
              if (!exists) {
                ...
                if(_resultItems.first['error'] == null){
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => ResultPage1(data: _resultItems.first, message: '', isSuccess: true,)),
                  );
                }else{
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => ResultPage1(data: _resultItems.first, message: _resultItems.first['error'], isSuccess: false,)),
                  );
                }
                setState(() {});
              }
            ...
            print("UUID does not match. Skipping this record.");
          }
        } else {
          print("Nothing new");
        }
...
// Function to pick and load the JSON file
  Future<File?> loadJsonFile() async {
    setState(() {
      _isLoadingJson = true;
      _statusMessage = 'Selecting JSON file...';
    });
    ...
    if (result != null) {
      ...
      setState(() {
        _jsonFileName = sanitizedFileName;
        _statusMessage = 'JSON file selected: $sanitizedFileName';
      });
  ...
  Future<void> uploadJsonFile(File file,String name,String dateformat) async {
    setState(() {
      _statusMessage = 'Uploading json file...';
      print(_statusMessage);
    });
    ...
    try {
      ...
      final response = await supabase.storage
          .from('raw') // Bucket name
          .upload('${name}/${dateformat}/${_jsonFileName}', file); // Path in the bucket
...
// Function to upload the JSON file to Supabase
  ...
  Future<void> insertIntoTable(String timestampz) async {
    ...
    final data = {
      ...
      'annotations_name' : _jsonFileName,
      'command': AudioProcessingMode.all.value,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: !_isProcessing ?  AppBar(title: Text('All Modes')) : null,
      body: Center(
            ...
            children: [
              ...
              if(_jsonFile == null)...[
                Container(
                  ...
                  height: MediaQuery.of(context).size.height * .1,
                  child: ElevatedButton(
                    onPressed: _isLoadingJson ? null : ()async {
                      _jsonFile= await loadJsonFile();
                    },
                    style: ElevatedButton.styleFrom(
                      padding: EdgeInsets.symmetric(horizontal: 40, vertical: 16),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Container(
                          width: MediaQuery.of(context).size.width/1.5,
                          child: Text("${_jsonFileName}",overflow: TextOverflow.ellipsis,)
                      ),
                      Spacer(),
                      IconButton(
                        onPressed: (){
              ...
              ElevatedButton(
                onPressed: _audioFileName.isEmpty || _jsonFileName.isEmpty ? null : (){
                  setState(() {
                    _isUploading=true;
                    print(_statusMessage);
                  ...
                  uploadJsonFile(_jsonFile!,tempName,dateFormat);
